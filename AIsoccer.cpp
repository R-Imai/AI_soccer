...
struct P_data G_go(struct P_data play){		//Aチーム用しかないので修正必要()
	...
	else	//このへんもplay.re=1にしないと汎用性がない	
	{
		play.cnd = 4;
	}
	return play;
...
struct P_data turn(struct P_data play, double ang){//回転関数(引数はdeg)
	...
	if (fabs(ang_path) >= 0.2){//どっち周りが近いか計算(この辺を戦略にするなら新しく関数作る)
		play.v = 0;
		if (ang_path<0){		//反時計回り
...
struct P_data turn_P(struct P_data turner, struct P_data waiter){//turnerがwaiterを向く関数
...
struct P_data pass(struct P_data passer, struct P_data getter){//passerがgetterへボールをける関数
	double pass_ang,ang;
...
struct P_data get(struct P_data play){//ボールを取りに行く関数
...
struct P_data turn_G(P_data play){//ゴールを向く関数(これもAチームのしかないので修正)
	double ang;
...
struct P_data shoot(struct P_data play){//シュートする関数(ただ思いっきりけるだけ)
	if (play.have = 1){
		play.v = 0;
		play.vang = 0;
...
void A_stop(void){//A全員をストップ(オーバーロードで個別にも止まれるようにする?)
	int n = 1;
	while (n <= 4){
		A[n].v = 0.0;
		n++;
